...
public class Palabras {
    ...
    public void introducir(String entrada){
        if (entrada.split(", ").length>0){
            ...
            for (int i = 0 ;i < this.lista.length;i++){                 
                this.lista[i]=this.lista[i].strip();                    //quita posibles espacios en blanco antes y después de cada palabra.
            }
            if(this.lista.length==1){                                   //si sólo se introducen palabras vacías, se deja la lista como null.
                if(this.lista[0].isBlank()) this.lista = null;
            }
        }
    }
    ...
    public String[] filtrar(String filtro){
        /* 
         * Filtra la lista de este objeto y devuelve la nueva lista con las palabras que cumplan el filtro.
        ...
        if (this.lista!=null && !(filtro.isEmpty())){
            ...
            for (int i = 0;i<this.lista.length;i++){ //recorremos todas las palabras.
                ...
                if ((filtroChars.length<=tempChars.length) && (tempChars[0] == filtroChars[0])){//si el filtro cabe en la palabra y la primera letra concuerda.
                    sw=true;
                    for(int n = 1 ;(n < filtroChars.length) && (sw);n++){//recorremos la palabra comparando con el filtro. 
                        if (tempChars[n]!=filtroChars[n]){  //si en algun momento el filtro no concuerda se cambia sw a false.
                            sw=false;                       //lo que provoca que se pare el bucle para esa palabra y no se añada a la lista.
                        }
                    }
                    if (sw) res=res+","+this.lista[i]; //si al recorrer la palabra sw sigue siendo true, el filtro concuerda, se añade la palabra.
                    tempChars.toString();
                }
            }  
        }
